...
//   bench_build -n 1000000 -alpha 4 -d 2 -data uniform -rounds 3 [-seq 1] [-base 1024]
// Vary PARLAY_NUM_THREADS for scaling (bench/scaling.sh).
#include <algorithm>
#include <cstdio>
...
struct Build {
  static void run(const bench::Args& args) {
    ...
    if (seq) {
      ...
      std::printf("  sequential      %.3f s   (%.2f M walk steps)\n", best_seq, S.stats().steps / 1e6);
    }
    for (int r = 0; r < rounds; r++) {
      bench::Timer t;
      S.build_parallel(base);
      best_par = std::min(best_par, t.seconds());
    }
    const BuildStats& st = S.stats();
    std::printf("  parallel        %.3f s   (%.2f M walk steps, %zu merges, forest depth max %zu)%s\n", best_par,
                st.steps / 1e6, st.merges, st.max_forest_depth,
                seq ? ("  speedup " + std::to_string(best_seq / best_par).substr(0, 5) + "x").c_str() : "");
    size_t lists = 0, bytes = 0;
    for (idx_t i = 0; i < S.n(); i++) {
      const FingerLists& F = S.lists(i);
      lists += F.num_lists();
      bytes += F.entries.size() * sizeof(Entry) + F.radius.size() * sizeof(dist_t) + F.size.size();
    }
